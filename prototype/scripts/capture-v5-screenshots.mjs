...
const outputDir = join(rootDir, "screenshots", "v5", "current");
...
async function run() {
  ...
  await page.getByTestId("screen-chat").waitFor({ timeout: 3000 });
  await captureShell(page, "review-03-bound-chat-accepted.png", "共同聊天 + 事件抽屉");
  await capture(page, "app-11-event-expanded-accepted.png", "已约好事件展开态");
  await clickButton(page, "等待回应");
  await clickButton(page, "接收方");
  await capture(page, "app-12-receiver-needs-action.png", "阿川视角收到提议");
  await clickButton(page, "发起方");
  await clickButton(page, "对方轻改");
  await capture(page, "app-13-initiator-countered.png", "小雨看到阿川轻改");
  await clickButton(page, "已约好");
  await clickTestId(page, "agreement-complete");
  await capture(page, "app-14-event-collapsed-completed.png", "完成后收起为事件胶囊");
  await clickTestId(page, "event-capsule-park-walk");
  await capture(page, "app-15-event-detail-sheet.png", "事件详情 Sheet");
  await clickTestId(page, "event-sheet-save-memory");
  await capture(page, "app-16-memory-draft-from-event.png", "从事件进入小窝草稿");
  await clickTestId(page, "memory-confirm");
  await capture(page, "app-17-memory-pending-partner.png", "小窝等待另一半点头");
