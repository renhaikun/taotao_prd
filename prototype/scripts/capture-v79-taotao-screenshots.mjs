...
import { taotaoPageStates, v7TaotaoBoardVersion } from "../src/data/v7TaotaoBoardData.js";

const rootDir = dirname(dirname(fileURLToPath(import.meta.url)));
const outputDir = join(rootDir, "screenshots", "v7", "taotao", "current");
const baseURL = process.env.TAOTAO_BASE_URL ?? "http://127.0.0.1:5175";
const taotaoURL = `${baseURL}${baseURL.includes("?") ? "&" : "?"}mode=taotao`;
const shots = [];
...
async function run() {
  ...
  await page.goto(taotaoURL, { waitUntil: "networkidle" });
  await captureLocator(page.getByTestId("v7-taotao-board-overview"), "taotao-00-overview.png", `${v7TaotaoBoardVersion} overview`);

  for (const [index, state] of taotaoPageStates.entries()) {
